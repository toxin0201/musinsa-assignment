...
./gradlew test             # 단위 + 슬라이스 + 통합 테스트 254건
...
- 단위 · 슬라이스 · 통합 테스트 254건 전부 GREEN (`@DataJpaTest` 29건, `@WebMvcTest` 47건, `@SpringBootTest` 통합 144건, 순수 단위 33건, 애플리케이션 컨텍스트 로드 1건).
...
### 2.2 엔드포인트별 요청 · 응답 (빌드한 jar 를 띄워 curl 로 왕복한 실제 응답. 캡처 로그 파일은 제출물에 포함하지 않았다)
...
`Clock` 을 주입해 검증한다. curl 로는 시계를 조작할 수 없어, curl E2E 는 미만료 상태에서의 배분 · 제자리 복원
...
| 주문번호 비교 기준 | 공백을 포함해 보낸 값 그대로 정확 일치. 앞뒤 공백을 다듬지 않는다 | 호출 측이 보낸 값을 바꾸지 않고 저장해야 주문 시스템과 그대로 대조할 수 있음 |
...
| `@SpringBootTest` 통합(`AbstractPointIntegrationTest`) | 144 | 예시 A~E 전 과정, 반복 부분 취소 · 전액 취소, 수기 우선 · 만료 순 소진, 적립취소 조건, 개인 한도, 동시성(유실 없음 · 잔액 음수 없음 · 3초 타임아웃), 재적립 실패 롤백, 방어적 분기(GAP-1 회귀) |
